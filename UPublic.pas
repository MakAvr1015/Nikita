...
///  <summary>
///  Óíèâåðñàëüíàÿ ïðîöåäóðà èìïîðòà
/// </summary>
...
/// <summary>
/// Ïðîöåäóðà çàïîëíåíèÿ áóôåðà ïåðåòàñêèâàíèåì
/// </summary>
...
///<summary>
///  Ðàñïàðñèòü äîï.èíôî ïî ñòîëáöàì
///</summary>
procedure CalcFieldsDopInfo(p_data_set : TdataSet; p_dop_fld_name : String);
///<summary>
///  Äîáàâèòü ñòîëáöû äëÿ äîï.ïàðàìåòðîâ òîâàðà â Ãðèä
///</summary>
...
/// <summary>
///  Ïðîöåäóðà èìïîðòà äîêóìåíòîâ èç XML
///  </summary>
...
begin
  ...
  begin
    ...
    Caption:='Äîêóìåíòû ðåçåðâà ïî àðòèêóëó "'+p_article+'"';
    showAsChild;
  end;
end;
...
begin
  result := capt;
  if (Language = 'ÐÓÑ') or (Language = '') then
    exit;
...
begin
  ...
  begin
    caption := translateCapt('Æóðíàë ñïèñàíèé', Language, TranslateFile);
    tag := 2;
    showAsChild;
  end;
end;
...
//  except
    ...
    begin
      ...
      begin
        ...
        LogMsg('Íà÷èíàåì çàïóñê ïðèëîæåíèÿ');
        ...
        LogMsg('Óñòàíîâëåí ïàðîëü');
        try
          dm.pFIBDatabase.Connected := true;
          LogMsg('Ñîåäèíåíèå óñòàíîâëåíî');
          dm.pFIBDatabase.DefaultTransaction.Active := true;
          result := true;
          prg_title := BaseName.Text;
          LogMsg('Ïîäêëþ÷àåì ÊÊÌ');
          if Atol.Checked then
            kkm := TKkm.Create(1);
          if Strih.Checked then
            kkm := TKkm.Create(2);
          LogMsg('ÊÊÌ ïîäêëþ÷åíà');
        except
          on E: Exception do
          begin
...
begin
  ...
  // dxDockPanel.Caption:='Òîâàðíûå çàïàñû';
