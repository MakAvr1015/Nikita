/******************************************************************************/
/***          Generated by IBExpert 2011.01.11 31.05.2011 21:40:33          ***/
/******************************************************************************/
...
BEGIN
  ...
  begin
    select param_value from sp_get_sys_param('CHANGE_SKLAD_OST_STATE') into :f_sys_state;
    if (f_state=f_sys_state and f_state>f_old_state ) then
    begin
      if (not exists(select 1 from t_money_in_str where f_doc=:f_id)) then
        select F_MONEY_IN_ID from SP_T_DOC_OUT_MAKE_PAY(:f_id) into :F_MONEY_DOC;
    end
  end

--  SP_T_DOC_OUT_MAKE_PAY
END^
...
--==========================================================
  ...
  into
    :f_move_out;
  select
    coalesce(:f_move_out,0)+
    sum(case
      when d.f_sklad_from=:f_sklad then s.f_cnt
      else 0
    end),
    coalesce(:f_move_in,0)+
    sum(case
      when d.f_sklad_to=:f_sklad then s.f_cnt
      else 0
    end)
  from
    t_doc_move d
    inner join t_doc_move_str s on d.f_id=s.f_doc_move and (d.f_sklad_from=:f_sklad or d.f_sklad_to=:f_sklad )
      and s.f_good=:f_good and d.f_state>=(select param_value from sp_get_sys_param('CHANGE_SKLAD_OST_STATE'))
  where
    d.f_date>:f_reg_date
    and d.f_date<=:f_date
  into :f_move_out,:f_move_in;
--========================================================
/*
  select
